...
__global__
void gpu_sum_scan_naive(int* const d_out,
	...
	const size_t numElems)
{
	...
	int d_hist_idx = blockDim.x * blockIdx.x + threadIdx.x;
...
void sum_scan_naive(int* const d_out,
	const int* const d_in,
	const size_t numElems)
{
	int blockSz = MAX_BLOCK_SZ;
	int gridSz = numElems / blockSz;
	if (numElems % blockSz != 0)
		gridSz += 1;
	checkCudaErrors(cudaMemset(d_out, 0, numElems * sizeof(int)));
	...
	const size_t numElems)
{
	...
	if (grid_sz <= max_elems_per_block)
	{
		...
		checkCudaErrors(cudaMalloc(&d_dummy_blocks_sums, sizeof(int)));
		checkCudaErrors(cudaMemset(d_dummy_blocks_sums, 0, sizeof(int)));
	...
	//checkCudaErrors(cudaMemcpy(h_block_sums, d_block_sums, sizeof(int) * grid_sz, cudaMemcpyDeviceToHost));
	//std::cout << "Block sums: ";
