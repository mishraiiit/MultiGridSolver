...
int main(int argc, char * argv[]) {
	...
	auto neighbour_list = deepCopyMatrixCSRCPUtoGPU(A_CSRCPU);
	auto A_CSC = readMatrixGPUMemoryCSC(filename);
	...
	int * useful_pairs_cpu_prefix = (int *) malloc(A_CSRCPU->rows * sizeof(int));
	...
	computeRowColAbsSum <<<number_of_blocks, number_of_threads>>> (A_CSR, A_CSC, ising0, 8.0);
	...
	sortcomputation.tic();
	sortNeighbourList<<<number_of_blocks, number_of_threads>>> (A_CSR, neighbour_list, Si, allowed, 8, ising0);
	cudaDeviceSynchronize();
	sortcomputation.toc();

	TicToc aggregationtime("Aggregation time");
	aggregationtime.tic();
	aggregation_initial<<<number_of_blocks, number_of_threads>>> (A_CSRCPU->rows, paired_with);
	aggregation<<<number_of_blocks, number_of_threads>>> (A_CSRCPU->rows, neighbour_list, paired_with, allowed, A_CSR, Si, 0, ising0, bfs_distance);
	aggregation<<<number_of_blocks, number_of_threads>>> (A_CSRCPU->rows, neighbour_list, paired_with, allowed, A_CSR, Si, 1, ising0, bfs_distance);
	cudaDeviceSynchronize();
	aggregationtime.toc();

	TicToc get_usefule_pairs_time("Get useful_pairs time");
	get_usefule_pairs_time.tic();
	get_useful_pairs<<<number_of_blocks, number_of_threads>>> (A_CSRCPU->rows, paired_with, useful_pairs);
	...
	cudaMemcpy(useful_pairs_cpu_prefix, useful_pairs, sizeof(int) * A_CSRCPU->rows, cudaMemcpyDeviceToHost);
	prefix_sum.toc();
	...
	int nc;
	cudaMemcpy(&nc, useful_pairs + A_CSRCPU->rows - 1, sizeof(int), cudaMemcpyDeviceToHost);
	mark_aggregations <<<number_of_blocks, number_of_threads>>> (A_CSRCPU->rows, aggregations, useful_pairs);
	cudaDeviceSynchronize();
	cudaMemcpy(aggregations_cpu, aggregations, sizeof(int) * A_CSRCPU->rows, cudaMemcpyDeviceToHost);
	cudaMemcpy(paired_with_cpu, paired_with, sizeof(int) * A_CSRCPU->rows, cudaMemcpyDeviceToHost);
	...
	MatrixCSR * newA_gpu = spmatrixmult_cudaSparse(P_transpose_gpu, spmatrixmult_cudaSparse(A_CSR, P_gpu, cudasparse_handle), cudasparse_handle);
	MatrixCSR * newA = deepCopyMatrixCSRGPUtoCPU(newA_gpu);
	printf("%d %d %d\n", newA->rows, newA->cols, newA->nnz);
